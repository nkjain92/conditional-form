...
export default function Dashboard() {
  ...
  const [themes, setThemes] = useState<FormTheme[]>([{ name: '', maxVotes: 100 }]);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');

  const addTheme = () => {
    setThemes([...themes, { name: '', maxVotes: 100 }]);
  };
  ...
  const updateTheme = (index: number, field: keyof FormTheme, value: string | number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (themes.some(theme => !theme.name)) {
      setError('Please fill in all theme names');
      return;
    }
    ...
    <main className='min-h-screen p-8 bg-[#0A0A0A] text-white'>
      <div className='max-w-4xl mx-auto'>
        ...
        <p className='text-lg text-center mb-12 text-white/60'>
          Design your custom voting form with multiple themes
        </p>

        <form onSubmit={handleSubmit} className='space-y-8'>
          <div className='space-y-6 bg-white/5 backdrop-blur-lg p-8 rounded-2xl border border-white/10'>
            <div>
            ...
            <div className='space-y-4'>
              {themes.map((theme, index) => (
                <div key={index} className='flex gap-4'>
                  ...
                  <div className='w-32'>
                    <label className='sr-only' htmlFor={`maxVotes-${index}`}>
                      Maximum votes for {theme.name || 'theme'}
                    </label>
                    <input
                      type='number'
                      id={`maxVotes-${index}`}
                      value={theme.maxVotes}
                      onChange={e => updateTheme(index, 'maxVotes', parseInt(e.target.value))}
                      className='block w-full rounded-xl border border-white/10 px-4 py-3 bg-white/5 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-all'
                      min='1'
